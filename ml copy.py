...
def preprocess_and_predict(data):
    ...
    mensaje+="La persona tiene aproximadamente {:.2f}% de riesgo de tener una enfermedad cardiaca.".format(predicciones_nuevas[0] * 100)
    # print("La persona tiene aproximadamente {:.2f}% de riesgo de tener una enfermedad cardiaca.".format(predicciones_nuevas[0] * 100))
    
    # CRITERIO
    
    # Obtener la probabilidad base del nuevo paciente
    probabilidad_base = modelo_rf.predict_proba(data)[:, 1]
    X = joblib.load('X_data.joblib')
    # Calcular el impacto de cada característica
    impactos = {}
    for feature in X.columns:
        temp_df = data.copy()

        # Evaluar el impacto al cambiar el valor de la característica a la media o mediana
        valor_original = temp_df[feature].values[0]
        valor_medio = X[feature].mean()  # Puedes usar la media o la mediana
        temp_df[feature] = valor_medio

        # Obtener la probabilidad con el valor cambiado
        probabilidad_cambiada = modelo_rf.predict_proba(temp_df)[:, 1]

        # Calcular el impacto como la diferencia en la probabilidad
        impacto = abs(probabilidad_cambiada - probabilidad_base)[0]
        impactos[feature] = impacto

    # Crear un DataFrame para visualizar el impacto de cada característica
    ...
    # Mostrar el impacto de las características
    # print(impacto_df)
    mensaje+= "\n Importancia de Factores" +impacto_df+"\n Por lo tanto su resultado es:"
